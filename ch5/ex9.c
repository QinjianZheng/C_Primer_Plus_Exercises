...
可以使用==运算符将scanf()的返回值和1作比较，测试两 值是否相等。
*/
...
void temperatures(double fahrenheit) {
    ...
    const double ADJUST = 32.0;
    const double ADJUST_C_TO_K = 273.16;
    double celsius;
    double kelvin;
    celsius = SCALE * (fahrenheit - ADJUST);
    kelvin = celsius + ADJUST_C_TO_K;
    printf("%.2lfF = %.2lfC = %.2fK\n", fahrenheit, celsius, kelvin);
}